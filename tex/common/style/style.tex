...
\usepackage{hyperref}
\hypersetup{hidelinks,colorlinks=true,linktoc=all,breaklinks=true,allcolors=color2,bookmarksopen=false,pdftitle={\strStudyGuide: \subjectName},pdfauthor={Miguel Hernández-Cabronero <miguel.hernandez@uab.cat>}}
\usepackage{pdfcomment}
% [normalem] keeps \emph as italics (ulem's default redefines it to underline, which would also
% affect every \textit-via-\emph in the book, not just \concept/\conceptRef in alias.tex).
\usepackage[normalem]{ulem}
\setlength{\ULdepth}{0.1em} % dash sits further below the text, matching the site's own offset
\renewcommand{\ULthickness}{0.5pt} % \ULthickness is a plain macro (ulem.sty), not a length register
...
%     regular digital colors
    ...
    \definecolor{colorConcept}{RGB}{117,117,117} % \concept/\conceptRef (alias.tex): matches the site's gray (rgba(0,0,0,0.54) over white)
}{
%     brighter print colors
    ...
    \definecolor{colorConcept}{RGB}{117,117,117} % \concept/\conceptRef (alias.tex): matches the site's gray
}


\usepackage{graphicx} % Required for including pictures
\graphicspath{{common/style/}{common/fig/chapters/}{common/fig/missions/}} % Specifies the directory where pictures are stored
...
% The `schedule` package hardcodes its day-of-week header text to English
% (\@M@week etc., used only for display - the \BeginOn{Monday}/\ifx
% day-matching logic is separate and untouched by this). Overriding it here,
% right after \usepackage{schedule}, instead of in es/strings.tex's
% \AtBeginDocument (tried first): that made \makeatletter/\makeatother leak
% across the \AtBeginDocument hook queue and broke hyperref's PDF-bookmark
% string sanitizing (\ensureascii) for an unrelated href elsewhere.
\makeatletter
\ifthenelse{\equal{\docLang}{es}}{%
  \def\@M@week{{Lunes} {Martes} {Mi\'ercoles} {Jueves} {Viernes} {S\'abado} {Domingo}}
}{}
\ifthenelse{\equal{\docLang}{ca}}{%
  \def\@M@week{{Dilluns} {Dimarts} {Dimecres} {Dijous} {Divendres} {Dissabte} {Diumenge}}
}{}
\makeatother
...
\usepackage{babel} % Language options are passed by <lang>/strings.tex
% Heading of the table of contents: babel's Spanish default is "Índice general"; <lang>/strings.tex may define \strContents to override it
\ifdefined\strContents\addto\captionsspanish{\renewcommand\contentsname{\strContents}}\fi
...
\fancyhead[L]{\strStudyGuide{\color{color1}:} \subjectName}
...
\newtheorem{notation}{\strNotation}[chapter]
...
\newtheorem{theoremeT}[dummy]{\strTheorem}
\newtheorem{problem}{\strProblem}[chapter]
\newtheorem{exerciseT}{\strExercise}[chapter]
\theoremstyle{blacknumex}
\newtheorem{exampleT}{\strExample}[chapter]
\theoremstyle{blacknumbox}
\newtheorem{vocabulary}{\strVocabulary}[chapter]
\newtheorem{definitionT}{\strDefinition}[section]
\newtheorem{corollaryT}[dummy]{\strCorollary}
\theoremstyle{color1num}
\newtheorem{proposition}[dummy]{\strProposition}
