...
Modern communications are based on layered network stacks. 
...
\section{The TCP/IP stack}\label{sec:internet:the_tcp_ip_stack}


The \conceptRef{internet}{Internet} is a practical solution to the problem of global computer communication,
whose development started in the late 1960's -- early 1970's, and is still ongoing.

Packet switching was conceived during the early 1960s, and the first 
wide-area implementation, ARPANET, began in 1969. 
% 
A key challenge at the time was to interconnect existing networks, 
...
existing LANs, with the goal of offering networking 
...
\item[\textbf{Layer 2}] \textbf{(Network)}:
  Identify and communicate devices within the same LAN.\\
  The term ``Data Link'' layer (from the OSI model) is sometimes used.
\item[\textbf{Layer 3}] \textbf{(Internet)}:
  Identify and communicate devices across all LANs.\\
...
\item[\textbf{Layer 4}] \textbf{(Transport)}:
  ...
  \begin{enumerate}[label=\alph*)]
   ...
   and in order \\-- \textit{but} an overhead is introduced 
...
\section{TCP/IP protocol overview}\label{sec:internet:tcp_ip_protocol_overview}

Multiple \conceptRef{protocol}{protocols} are defined to implement the functionality
of each layer. These definitions include the required packet format,
...
% 
Assuming SMTP uses headers in its PDU, how many headers will 
the corresponding Layer-2 PDU contain?
\end{exercise}
...
\section{Who is the Internet boss?}\label{sec:internet:who_is_the_internet_boss}

The Internet Engineering Task Force (\conceptRef{IETF}{IETF}) was created in 1986
to develop and publish standards to be used in the Internet. 
% 
Their main output are  ``Request for Comment'' (\conceptRef{RFC}{RFC}) documents,
...
they are adopted by users. RFCs can be updated and retired for this reason.
