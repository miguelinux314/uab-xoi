...
\chapter{The analog-digital veil}\label{sec:piercing}
...
Computers work with and transmit digital \concept{data}, \ie, sequences of binary
\mbox{\zero\ and \one} symbols. 
However, data transmission happens in the real world, where \mbox{\zero\ and \one} are ideas and not tangible objects.
...
although they are often used interchangeably. In technical contexts, they should be used and interpreted accurately.
% 
\begin{exercise}
...
represented by numbers. For instance, one could use \concept{ASCII} encoding to represent the string ``Bobby''
...
\begin{itemize}
\item integer or with decimals? 
\item \concept{bitdepth}? (\eg, 8 bits per number)
\item \conceptRef{sign}{signed} or \conceptRef{sign}{unsigned}?
\item For multibyte numbers: \conceptRef{big endian/little endian}{big endian} or \conceptRef{big endian/little endian}{little endian}?
\item For numbers with decimals: \conceptRef{fixed-point/floating-point}{floating-point} or \conceptRef{fixed-point/floating-point}{fixed-point}?
\end{itemize}
% 
\begin{exercise}
Explain how to complete the second row given the first (and vice-versa),
...
\begin{exercise}
Consider the following message, which is composed of a concatenation 
of 3-bit unsigned integers: \otherBase{a4 3f 20}.
\begin{itemize}
\item How many bits and bytes long is it?
\item What are the first five integers contained in the message?
\end{itemize}
\end{exercise}
...
Popular choices include using voltage on copper wires, sending light over optical fiber and manipulating the 
electromagnetic field using antennas.

\begin{exercise} Ponder:\\[-0.5cm]
\begin{itemize}
\item Can we make a copper cable as long as we desire?
\item What medium do Wi-Fi connection use? 
\item Do we need optical fibers to perform light-based communication?
\end{itemize}
\end{exercise}


If we opt for copper wires, we can put several in \conceptRef{parallel/serial}{parallel} and send more data at the same clock speed.
This, however, comes at the price of additional complexity and cost than \conceptRef{parallel/serial}{serial} designs. 
Voltage interferences in the data lines may happen for a number of reasons, 
...
Optical fiber cables work of this phenomenon, called refraction.
...
In a single-mode optical fiber, the sensor at the receiving end 
...
monochrome components, which are sensed separately. 
In this way, multi-modal optical fiber allows \conceptRef{multiplex}{multiplexing}
...
What happens at the other end if we turn this cone of light on and off very quickly?
...
Even though there are many classes of antennas, their main purpose 
...
A dipole antenna like the one to the right uses this concept by applying an alternating voltage $V$,
...
will be in the ballpark of $E / r^2$.

This is sometimes referred to as the inverse square law, and is
the reason why currents inducted in the receiver's antenna are usually between nanovolts 
...
pierced. Once way of doing this is modulation: a base sinusoidal signal called carrier is produced,
and the data are encoded by modifying this carrier. For instance, one can change the amplitude of the carrier (ASK),
its frequency (FSK), or even the amplitude and the phase at the same time (QAM). 

\begin{exercise}
The figure above displays an example of amplitude modulation and of frequency modulation. 
Which is which? For each case: is that modulation transmitting an analog or a digital signal?
\end{exercise}


\begin{exercise}
What's the difference between bandwidth and transmission speed? 
