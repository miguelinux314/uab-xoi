...
\item \textbf{Gateway/via}: if the row corresponds to an \conceptRef{direct/indirect delivery}{indirect delivery}, this column contains the IP address
  of the next router. Otherwise, this column contains \otherBase{0.0.0.0} to indicate a \conceptRef{direct/indirect delivery}{direct delivery}.\\[-0.3cm]
...
\item The original datagram's header is copied into the fragments' with the following modifications:
  ...
  \item The \concept{Offset} field in each fragment indicates the position (byte index)
    ...
    %     
    Given an offset in bytes, the \concept{Offset} field is calculated dividing by $8$.
...
The Address Resolution Protocol (ARP) is used by Layer~3 (\concept{IP})
...
\begin{remark}
The IP queried by the Layer~3 is that of the router for \conceptRef{direct/indirect delivery}{indirect delivery} and of the destination device
for \conceptRef{direct/indirect delivery}{indirect delivery}. Also remember that the datagram's destination IP does not change during forwarding.
\end{remark}
...
\begin{itemize}
\item The node needs to perform the \conceptRef{direct/indirect delivery}{indirect delivery} of a datagram,
...
\conceptRef{direct/indirect delivery}{indirect delivery} with \concept{TTL}$=1$ (see Section~\ref{sec:layer3:routing}).
\end{itemize}
