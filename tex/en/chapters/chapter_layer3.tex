...
The \conceptRef{IP}{Internet Protocol (IP)} allows exchanging datagrams 
(Layer~3 PDUs) between devices from different LANs.
% 
Its addressing system identifies devices globally and makes it possible to 
route datagrams anywhere we need. 

Devices communicating via IP may belong to LANs
based on different technologies and with different types of MAC address.
This has two implications:
\begin{itemize}
  \item A datagram that ``fits'' in the MTU of the source LAN
  ...
  but not necessarily its MAC address. Only if we are part of the destination
  LAN we will need that MAC. IP uses ARP to translate known IP addresses
  to MAC addresses of the destination LAN's type.
\end{itemize}


Datagrams may travel through networks not controlled 
...
(\eg, to implement TCP's data streams).
...
% 
For many years, the Internet has been struggling to transition towards IPv6, 
but the process is not complete and only IPv4 provides global coverage.

IPv4 delegates in the ARP protocol the translation of known IP addresses 
into MAC addresses within each LAN. ARP does \textit{not} use IP datagrams.

The following protocols encapsulate their PDUs in the payload of IP 
datagrams:\\[-0.6cm]
\begin{itemize}
...
\section{Layer~3 addressing}\label{sec:layer3:addressing}

IPv4 addresses are $32$~bits long. They are most often presented to humans in
the \concept{quad decimal} format, \eg,
\begin{center}
\otherBase{142.250.201.67}
\end{center}

IPv6 addresses are $128$~bits ($12$~bytes) long, \ie, 
...
\begin{remark}
When an IPv6 address is used as part of an URL, it is surrounded with brackets,
\eg, \url{https://[fe80::f524:a73a:e946:7c02]/}.
\end{remark}


\subsection{Public and reserved addresses}\label{sec:layer3:public_reserved}

Most IP addresses are \conceptRef{public/private IP}{public} and unique across the Internet, 
\ie, the previous IP has the same meaning worldwide: it identifies a single connected device.
% 
Many addresses are \conceptRef{public/private IP}{reserved/private} and can only be 
...
\begin{itemize}
\item How many different IPv4 addresses are there (including reserved and private)? 
...
\item Which of these corresponds to link local addresses?
\end{itemize}
\end{exercise}
...
\subsection{Blocks of IP addresses}\label{sec:layer3:ip_blocks}
...
An IP network (a LAN) is a block of addresses, but not all those addresses 
...
\item The address with all node bits set to \otherBase{1}: it identifies broadcast within the LAN.
\end{itemize}
...
\begin{remark}
All routers need a valid IP address per LAN to which they are connected.
...
\concept{IP} defines a packet format with a header 
...
\subsection{Operation}\label{sec:layer3:ip:operation}
...
The datagram is passed first to the source's router.
...
Routers are the devices in charge of forwarding IP datagrams
by looking at the destination IP address of each one, present in the IP header. Routers don't need 
to decapsulate headers of layers 4 and above. Therefore, they network stacks need contain only up to Layer~3
...
\item The router creates and sends a frame $F'$ making the following two changes to $F$:\\[-0.25cm]
  ...
  \item The Layer~2 header in $F'$ changes entirely:
    \begin{itemize} 
    \item The destination MAC is that of the next hop,
    ...
    and its MAC is obtained from that IP using ARP (Section~\ref{sec:layer3:arp}).\\[-0.4cm]
    %     
  ...
  \item The IP header in $F'$ differs from that of $F$ in the following:
    ...
    \item When the Options field is present, it is often updated.
...
When a router receives an IP datagram to forward and that router 
is connected to the same LAN as the destination IP address, the router performs a \conceptRef{direct/indirect delivery}{direct delivery} (DD).
% 
Otherwise, the datagram is forwarded to the next router in an \conceptRef{direct/indirect delivery}{indirect delivery} (ID).
% 
...
\item \textbf{Gateway/via}: if the row corresponds to an indirect delivery, this column contains the IP address
  of the next router. Otherwise, this column contains \otherBase{0.0.0.0} to indicate a direct delivery.\\[-0.3cm]
...
\item How would this router forward a datagram directed to a public IP address?\\[-0.3cm]
...
This is sometimes not the case, which can cause routing errors. Two important problems 
...
\item ``Time to Live exceeded'': every time a datagram is forwarded,
  its TTL is decreased, as mentioned in Section~\ref{sec:layer3:ip:format}.
  ...
  When the TTL reaches 0, the datagram is discarded and this error is produced.
...
LANs can be regarded as address blocks (\eg, expressed in \concept{CIDR} format)
...
have the same public IP, no two LANs can contain the same address
(unless the address is private).

The $2^{32}$~addresses available in IPv4 are divided in blocks by means of 
...
can be split into two \otherBase{/1} blocks, each with half the IPv4 addresses 
...
\item What is the broadcast IP in each subnet?
\end{itemize}
\end{exercise}


% 
\section{IP fragmentation}\label{sec:layer3:ip_fragmentation}
\label{sec:layer3:fragmentation}

Before a device (including routers and regular computers)
can send a datagram through a LAN, it needs to verify that its length in bytes 
is not larger than that LAN's MTU. Otherwise, the datagram needs to be 
...
more fragments imply a larger overhead. Each of these fragments is sent 
...
\item The \conceptRef{DF}{DF flag} (don't fragment) of the original datagram is checked.
    If DF=\otherBase{1} but fragmentation is needed, the datagram is not fragmented 
    and an error is produced.\\[-0.2cm]

\item Payload data are divided in chunks of size at most $S$, so that $S + H = MTU$ 
...
\item The original datagram's header is copied into the fragments' with the following modifications:
  \begin{itemize}
  ...
  \item The \conceptRef{offset}{Offset} field in each fragment indicates the position (byte index)
    ...
    %     
    Given an offset in bytes, the \conceptRef{offset}{Offset} field is calculated dividing by $8$.
    Therefore, fragment offsets can only be multiples of $8$.\\[-0.3cm]
    
  \item The \conceptRef{MF}{MF flag} (more fragments) is set to \otherBase{1} (enabled)
...
\begin{exercise}
Given the \conceptRef{offset}{Offset} and the MF flag, how can we differentiate
...
The Address Resolution Protocol (\concept{ARP}) is used by Layer~3 (IP)
to obtain the destination MAC (physical) address corresponding to a known IP address.
...
\begin{remark}
The IP queried by the Layer~3 is that of the router for indirect delivery and of the destination device
for indirect delivery. Also remember that the datagram's destination IP does not change during forwarding.
\end{remark}

\subsection{Packet format}\label{sec:layer3:arp:packet_format}
...
\begin{remark}
The ARP protocol does \textit{not} use IP headers.
\end{remark}
\begin{remark}
...
\subsection{Operation}\label{sec:layer3:arp:operation}

When a node \node{A} needs to translate an IP address into a MAC address, it sends a broadcast frame
...
\begin{itemize}
\item The node needs to perform the indirect delivery of a datagram,
...
\conceptRef{arp cache}{cache} adding (or updating) the received IP $\leftrightarrow$ MAC translations.
% 
...
\begin{remark}
ARP can be used to scan for used IPs in a LAN.
...
and modern switches will filter ARP messages not matching the
...
protocol (technically part of IP) that is used to test and debug
(inter)network communications. 


\subsection{Packet format}\label{sec:layer3:icmp:packet_format}

ICMP messages are encapsulated as the payload of IP datagrams.
In the IP header, the Payload protocol is set to \otherBase{0x01} (``ICMP'').\\[-0.2cm]
...
\toprule
$8$ & $0$       & No & Echo Request ICMP message (\concept{ping}). \\
$0$ & $0$       & No & Echo Reply ICMP message (\concept{ping} reply). \\
...
\subsection{Operation}\label{sec:layer3:icmp:operation}

ICMP error messages are sent by routers and regular hosts,
...
a closed port (see Section~\ref{sec:layer4}), or simply the destination
host not being turned on.

\item The ``TTL exceeded'' message is sent when a router receives a datagram for 
indirect delivery with TTL$=1$ (see Section~\ref{sec:layer3:routing}).
\end{itemize}
