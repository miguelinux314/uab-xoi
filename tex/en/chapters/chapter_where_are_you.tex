...
\chapter{Addressing}\label{sec:where}
...
with just $2$ \conceptRef{network card}{network cards} (also: network controller, NIC) and $1$ cable, point-to-point connections are viable for $N=2$ devices.
...
in a \textit{complete graph}. Now every device needs to handle $N - 1$ cables to the other devices, 
...
\begin{remark}
$O(N^2)$ is notation for a quadratic complexity bound. 
This is \textit{not} the same as exponential and should never be confused.
% 
...
There is one trick up our sleeves: data buses, where one or more data lines
...
a collision that makes data transmission impossible while it lasts.\\[-0.5cm]

\begin{remark}
As computer networking was developed, people experimented with many alternatives to the 
bus \concept{topology}. One curious example is Token Ring, where devices
...
They worked similarly, but they offered pre-built \conceptRef{physical port}{physical ports} 
...
around a switch is called a star topology.
...
Once again, the main advantage of data buses is also their main weakness: 
all devices send and receive data using the same data line. 
...
In this case, we need to make sure we can route our messages
...
In this scenario, you want the \conceptRef{client/server}{client} and \conceptRef{client/server}{server} of the alarm monitoring 
...
inside an operating system (OS). Some of these identifiers are
referred to as addresses, precisely because they are used 
to find and reach a remote device. 


Addresses are typically \textit{numerical} identifiers, that is, just a number.
As such, they can be expressed in different bases, including decimal and hexadecimal.
...
\begin{exercise}
How many elements can be uniquely identified (at most) with an address we express 
using 5 bits? How about 10? How about 20? Express the general solution mathematically.
\end{exercise}

\begin{exercise}
Suppose there are $1000$~machines in a LAN. How many address bits are needed?
...
\section{How do I get there?}\label{sec:where:route}
...
the destination, \ie, to route our messages through the graph of 
...
\begin{itemize}
 ...
 \item What's the minimum number of messages sent so that \node{3} can contact \node{7},
...
\conceptRef{swimlane diagram}{Swimlane diagrams} like the one below are used to represent the interactions
...
\begin{exercise}
Based on your answer to Ex.\ref{ex:how_many_packets}, produce a swimlane diagram
