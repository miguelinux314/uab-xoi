...
such as \otherBase{ietf.org} or\\\otherBase{cv.uab.cat} into IP
addresses that can placed inside datagrams.

DNS services are run on both TCP and UDP, \ie, both transport protocols
...
% 
Second-level domains can then structure their \concept{name zone}
...
\subsection{Records}\label{sec:layer7:dns:records}

DNS can be seen as a distributed database with different types of entries
called \conceptRef{DNS record}{DNS records}. The main ones are:

\begin{itemize}
\item[\textbf{A}:] An \conceptRef{A}{A record} maps a domain name to an IPv4 address.
\item[\textbf{AAAA}:] An \conceptRef{AAAA}{AAAA record} maps a domain name to an IPv6 address.
\item[\textbf{NS}:] An \conceptRef{NS}{NS record} points to a DNS server that can help.
\item[\textbf{MX}:] A \conceptRef{MX}{MX record} provides the domain name of the email server
  that handles messages for a domain.
\item[\textbf{CNAME}:] A \conceptRef{CNAME}{CNAME record} maps a domain name to another domain name.
\end{itemize}
...
the requested A or MX records,
but the client may also receive a redirection to another DNS (NS)
or an alias to another domain name (CNAME).

\subsection{Resolution}\label{sec:layer7:dns:resolution}

The distributed and hierarchical nature if the DNS database makes it 
sometimes necessary to exchange multiple messages to satisfy a \concept{DNS query}.
 
For instance, Alice may perform a fully \conceptRef{dns resolution}{iterative resolution} all on her own.
The first two replies contain an NS record pointing to the next
...
whose job is to receive Alice's requests and resolve them for her in a recursive
manner.
...
The combination of \conceptRef{dns resolution}{recursive resolution} and \conceptRef{dns cache}{cache} tables makes
...
unless they are renewed (see below). Addresses that are not renewed are put
...
After $T/2$ is elapsed, clients may try to \conceptRef{renew}{renew} the leased configuration. To do so,
...
\section{Autonomous Systems}\label{sec:layer7:as}

Routing is performed one hop at a time, using \conceptRef{routing table}{routing tables} 
...
\subsection{Organization}\label{sec:layer7:as:organization}
...
\subsection{Routing table configuration}\label{sec:layer7:as:routing_table_configurati}
...
RIP implements a variation of the Bellman-Ford shortest path algorithm to minimize the number of hops
...
\begin{itemize}
\item RIP uses a single number (the distance, \ie, the number of hops)
to guide a shortest-path graph algorithm. BGP uses \conceptRef{path}{paths} instead of distances.
This allows BGP to choose routes with more hops, but more desirable to the AS,
