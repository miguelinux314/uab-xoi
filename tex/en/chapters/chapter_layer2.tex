...
and identify each other within a local network (LAN).
% 
...
that can be inserted in a Layer~2 protocol is called the Maximum Transmission Unit (MTU),
\eg, $1500$~bytes for Ethernet.

Some Layer~2 protocols may also provide user authentication and
cryptographic protection;
\conceptRef{wi-fi}{Wi-Fi} is a notable example,
...
Many protocols have been and are being defined within Layer~2. Most importantly nowadays:\\[-0.65cm]
\begin{itemize}
\item \conceptRef{ethernet}{Ethernet} (IEEE 802.3), addressed in \secref{sec:layer2:ethernet}.
\item \conceptRef{wi-fi}{Wi-Fi} (\eg, IEEE 802.11be aka Wi-Fi 7, of 2024).
\end{itemize}

Within TCP/IP, Layer~2 encapsulates the following protocol PDUs
(both addressed in Chapter~\ref{sec:layer3}):\\[-0.65cm]
\begin{itemize}
  \item \concept{IPv4} and \concept{IPv6} datagrams.
  \item ARP messages.\\[-0.75cm]
\end{itemize}
...
\section{Layer~2 addressing}\label{sec:layer2:layer_2_addressing}
...
for a full description of reserved MAC addresses, including \conceptRef{unicast/multicast}{multicast}).
...
\subsection{Packet format}\label{sec:layer2:ethernet:packet_format}
Ethernet Layer~2 defines the following format, used in all messages.
Packets of this type are called frames:\\[-0.25cm]
...
\item \textbf{Payload type}: Identifier for the protocol 
  encapsulated in the payload,\\\eg, 
...
\item \textbf{Payload}: Content requested to be sent, \eg, by IPv4 or ARP.
  The maximum length of this field, the MTU, is $1500$~bytes for ethernet.
\end{itemize}

\begin{exercise}\ \\[-0.5cm]
\begin{itemize}
\item How many different MAC addresses are there in Ethernet (including reserved blocks)?
...
\newpage
\subsection{Operation}\label{sec:layer2:ethernet:operation}

Messages within an Ethernet LAN are normally sent in \conceptRef{unicast/multicast}{unicast} mode, \ie, they are one-to-one messages.
This allows switches to minimize power and \concept{bandwidth} consumption.
% 
In \conceptRef{unicast/multicast}{unicast}, the source device node must know the MAC of the destination.
% 
\begin{center}
...
A node will also discard a frame with an unsupported Payload Type (\conceptRef{ethertype}{EtherType}) field. 
% 
If the payload type is supported, it is used to decide what part of the OS receives the message,
\eg, the \concept{IPv4} stack or the ARP subsystem.

\begin{exercise} Continuing the example of the figure above, the network card of Node~B receives
a frame that begins with the following bytes. Should it be accepted by B?
\begin{center}
...
\begin{exercise}
The following scripts exemplify how to send and receive raw \conceptRef{ethernet}{Ethernet} frames. 
The client sends $5$ identical frames of \conceptRef{ethertype}{EtherType} \otherBase{0x1234} and then exits.
The server waits until it receives $5$ Ethernet frames of that type (checked in lines 12-13) and then exits.
% 
\begin{itemize}
\item Change the client's MAC address and interface name with yours.
\item Make the server reject frames not addressed to it.
\item Extend these scripts to implement a simple application with LAN capabilities.\\
  Suggested examples:
    \begin{itemize}
    \item A Layer-2 echo service that distinguishes between petitions and responses.
    \item A Layer-2 peer-to-peer chat that includes the origin's nickname in all messages.
    ...
    \end{itemize}
\item What is missing so that your application can connect to the rest of the Internet beyond your LAN?
\end{itemize}
