...
addressing and correct \concept{sequencing} (introduced in the previous chapters).
...
To coordinate layers and enable virtual communication between them,
each layer defines a different packet format, which \conceptRef{encapsulation}{encapsulates} 
a packet of the layer above it.


\section{Sending data}\label{sec:stack_layers:send}
...
the packet format, adding some overhead (typically a header) to the payload, \eg:
...
The PDUs of each layer have specific names such as \concept{frame}, \concept{datagram} or segment.
These names must be used with precision in technical contexts.

Each layer includes the \concept{PDU} of the layer above inside its PDU's packet format definition.
...
\begin{itemize}
    \item What is the payload size and the overhead size for each \concept{layer}'s \concept{PDU}?
    \item What is the payload size and the overhead size of the whole \concept{stack}?
\end{itemize}
\end{exercise}

\section{Receiving data}\label{sec:stack_layers:receive}

When a packet reaches the network \concept{stack} of the recipient, it is processed in reverse order,
\ie, layers are traversed from bottom to top. Each layer inspects \textit{only} the header corresponding to that layer.
If everything is correct, the layer \conceptRef{decapsulate}{removes the header} and passes the remainder of the packet to the layer above.
In turn, the layer on top passes the remaining data (the payload) to the recipient.
...
% 
Thanks to this, there exists a virtual communication between each layer in one end and its counterpart in the other
...
Layer L1 receives a packet of data and its \inlineCode{receive} function returns the following data
(assume unsigned integers and big endian order when needed:
\begin{center}
\otherBase{01 23 00 0a 01 00 00 06 00 aa 00 bb 43 20}
\end{center}
\begin{itemize}
\item What data payload was sent to the recipient program in this packet?\\[-0.25cm]

\item If this is the last packet of the communication, how much (payload) data was sent
...
(the \concept{physical layer}) of the \conceptRef{stack}{network stack} of Exercise~\ref{ex:how_it_is_done:three_layer_network}.
...
\begin{remark}
Python's \inlineCode{struct} library is useful, but not mandatory, to format packet bytes.
\end{remark}
