...
\chapter{Packets}\label{sec:packets}
...
using a parameter called Maximum Transmission Unit (\conceptRef{MTU}{MTU}), \eg, 1500~bytes in Ethernet.


Packet transmission across a data link must be done carefully,
particularly when that link is a bus shared by multiple devices.
...
\item \textit{Escape sequences}: Certain binary escape sequences 
...
\begin{remark}
It is also possible to signal the \textit{beginning} of a packet. In that case, a preceding sequence called preamble is used.
\end{remark}
...
A single data line must be multiplexed for multiple control commands
as well as multiple data streams. The priority is efficient power and buffer usage.

\item Multiple devices share a bus. They occasionally send messages of different lengths,
but not a huge volume of data. The priority is cost.

\item Multiple devices share a bus. They continuously send messages of different lengths,
trying to maximize the effective transmission rate through the channel. 
...
Other formats may include a trailer (also known as tail, most often used for error detection
...
is strictly defined so that it can be easily produced and parsed.
...
byte boundaries. Fields may also have length equal to $1$~bit,
in which case its is normally called a \conceptRef{flag}{\texttt{flag}} or flag bit.
\end{remark}
...
\begin{exercise}
The following packet contents were sniffed out of a network,
expressed in hexadecimal. Assuming the packet has the format of the example above:
% 
\begin{itemize}
...
\item Extend the packet format so that 
 \begin{itemize}
   ...
   \item The address of the destination device can be encoded
  \end{itemize}
...
In the code above, files are open in binary mode using modes \inlineCode{'rb'} and \inlineCode{'wb'}.

When open in binary mode, file bytes are directly represented by a \inlineCode{bytes} object, an array of 
integers between $0$ and $255$. In text mode, those bytes are processed (decoded) further by the \inlineCode{open} method,
...
to imagine communication as a \conceptRef{message/stream}{stream}, \ie, as a conceptual pipe where we put 
...
\conceptRef{packet}{Packets} often need to be forwarded through multiple networks. Routers are not perfect
...
If we want to simulate a \conceptRef{message/stream}{stream} of data, packets must contain enough meta-information in them so that losses can be detected and 
...
so some applications base their network communication in \conceptRef{message/stream}{messages} instead of streams.
