...
\chapter{Paquets}\label{sec:packets}
...
conté dades d'\conceptRef{overhead}{overhead} (sobrecàrrega, com ara les adreces que es tracten a la secció~\ref{sec:where}) que cal
enviar a més de les dades de payload (càrrega útil) de l'usuari. Les xarxes fixen la longitud màxima dels paquets
mitjançant un paràmetre anomenat unitat màxima de transmissió (\conceptRef{MTU}{MTU}), \eg, 1500~bytes a Ethernet.


La transmissió de paquets a través d'un enllaç de dades s'ha de fer amb cura,
sobretot quan aquest enllaç és un bus compartit per diversos dispositius.
...
\item \textit{Seqüències d'escapament}: certes seqüències d'escapament binàries 
...
\begin{remark}
També és possible senyalitzar l'\textit{inici} d'un paquet. En aquest cas, es fa servir una seqüència prèvia anomenada preàmbul.
\end{remark}
...
Una sola línia de dades s'ha de multiplexar per a diverses ordres de control
i també per a diversos fluxos de dades. La prioritat és un ús eficient de l'energia i del buffer (memòria intermèdia).

\item Diversos dispositius comparteixen un bus. De tant en tant envien missatges de longituds diferents,
però no un volum enorme de dades. La prioritat és el cost.

\item Diversos dispositius comparteixen un bus. Envien contínuament missatges de longituds diferents,
intentant maximitzar la taxa de transmissió efectiva a través del canal. 
...
Altres formats poden incloure un trailer o cua final (també conegut com a tail, que s'usa sobretot per a la detecció d'errors
...
està definit estrictament perquè es pugui produir i analitzar fàcilment.
...
límits de byte. Els camps també poden tenir una longitud igual a $1$~bit,
i en aquest cas normalment se'ls anomena \conceptRef{flag}{\texttt{flag}} o bit de flag.
\end{remark}
...
\begin{exercise}
El contingut de paquet següent s'ha capturat d'una xarxa,
expressat en hexadecimal. Suposant que el paquet té el format de l'exemple anterior:
% 
\begin{itemize}
...
\item Amplia el format de paquet de manera que 
 \begin{itemize}
   ...
   \item Es pugui codificar l'adreça del dispositiu de destinació
  \end{itemize}
...
Al codi anterior, els fitxers s'obren en mode binari amb els modes \inlineCode{'rb'} i \inlineCode{'wb'}.

Quan s'obren en mode binari, els bytes del fitxer es representen directament amb un objecte \inlineCode{bytes}, un vector 
d'enters entre $0$ i $255$. En mode text, el mètode \inlineCode{open} processa (descodifica) aquests bytes una mica més,
...
imaginar la comunicació com un \conceptRef{message/stream}{flux}, \ie, com una canonada conceptual on posem 
...
Sovint cal reenviar els \conceptRef{packet}{paquets} a través de diverses xarxes. Els routers (encaminadors) no són perfectes
...
Si volem simular un \conceptRef{message/stream}{flux} de dades, els paquets han de contenir prou metainformació perquè es puguin detectar les pèrdues i 
restaurar l'ordre correcte. Això introdueix un \concept{overhead} que no és adequat en tots els escenaris --\eg, latència molt baixa--,
de manera que algunes aplicacions basen la seva comunicació de xarxa en \conceptRef{message/stream}{missatges} en lloc de fluxos.
