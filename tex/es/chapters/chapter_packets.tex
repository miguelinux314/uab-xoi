...
\chapter{Paquetes}\label{sec:packets}
...
mediante un parámetro llamado unidad máxima de transmisión (\conceptRef{MTU}{MTU}), \eg\ 1500~bytes en Ethernet.


La transmisión de paquetes a través de un enlace de datos debe hacerse con cuidado,
en particular cuando ese enlace es un bus compartido por varios dispositivos.
...
\item \textit{Secuencias de escape}: ciertas secuencias de escape binarias 
...
\begin{remark}
También es posible señalar el \textit{principio} de un paquete. En ese caso se utiliza una secuencia previa llamada preámbulo.
\end{remark}
...
Una única línea de datos debe multiplexarse para varios comandos de control
y también para varios flujos de datos. La prioridad es un uso eficiente de la energía y del buffer (memoria intermedia).

\item Varios dispositivos comparten un bus. De vez en cuando envían mensajes de distintas longitudes,
pero no un volumen enorme de datos. La prioridad es el coste.

\item Varios dispositivos comparten un bus. Envían continuamente mensajes de distintas longitudes,
intentando maximizar la tasa efectiva de transmisión por el canal. 
...
Otros formatos pueden incluir un trailer (cola), también conocido como tail, que se usa sobre todo para la detección de errores
...
está estrictamente definido para que pueda producirse y analizarse con facilidad.
...
fronteras de byte. Los campos también pueden tener una longitud igual a $1$~bit,
en cuyo caso normalmente se le llama \conceptRef{flag}{\texttt{flag}} o bit de flag.
\end{remark}
...
\begin{exercise}
El siguiente contenido de un paquete fue capturado de una red,
expresado en hexadecimal. Suponiendo que el paquete tiene el formato del ejemplo anterior:
% 
\begin{itemize}
...
\item Extiende el formato de paquete para que 
 \begin{itemize}
   ...
   \item pueda codificarse la dirección del dispositivo de destino
  \end{itemize}
...
En el código anterior, los ficheros se abren en modo binario usando los modos \inlineCode{'rb'} y \inlineCode{'wb'}.
...
enteros entre $0$ y $255$. En modo texto, esos bytes son procesados (decodificados) además por el método \inlineCode{open},
...
imaginar la comunicación como un \conceptRef{message/stream}{flujo}, \ie, como una tubería conceptual donde ponemos 
...
\conceptRef{packet}{Los paquetes} a menudo tienen que reenviarse a través de varias redes. Los routers (encaminadores) no son perfectos
...
Si queremos simular un \conceptRef{message/stream}{flujo} de datos, los paquetes deben contener suficiente meta-información para que puedan detectarse las pérdidas y 
...
así que algunas aplicaciones basan su comunicación en red en \conceptRef{message/stream}{mensajes} en lugar de en flujos.
