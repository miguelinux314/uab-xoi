...
\subsection{IP routing}\label{sec:layer3:routing}
...
\item ``Time to Live exceeded'': every time a datagram is forwarded, its \concept{TTL} is decreased, as mentioned in Section~\ref{sec:layer3:ip:format}. 
...
\subsection{Subnets and supernets}\label{sec:layer3:subnetting}
\vspace{6em}
...
The process of creating multiple smaller LANs from a single address block is called \concept{subnetting}.
The inverse process, combining multiple LANs into a larger one is called \concept{supernetting}. 
